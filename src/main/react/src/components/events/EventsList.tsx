...
const formattedHomeVsAway = (event: Match) => <>{placeMapping[event.homeAway]} </>;
...
export const EventListItem = (props: {
        ...
        .catch((e) => {
          ...
          addAlert({
            message: `Trainer '${
              ...
              formattedDate(props.event.startTime)
  ...
  const handleCoachSelection = async (coach: string) => {
    return await matchesApiClient
      ...
      .then((e) => {
        console.debug("Coach update ", coach, " for event", props.event, ":", e);
        addAlert({
