...
class UserClient(
    ...
    fun createUser(user: CreateUser): User {
        ...
        val request = Request(POST, "/api/users").body(body)
    ...
    fun getUser(id: String): User {
        val request = Request(GET, "/api/users/$id")
        val response: Response = client(request)
        check(response.status.successful) {
    ...
    fun deleteAndValidateUser(id: String) {
        val request = Request(DELETE, "/api/users/$id")
        val response: Response = client(request)
        check(response.status.successful) {
            "Something went wrong deleting the user with id $id: ${response.bodyString()}"
        }

        val remainingUsers = getAllUsers()
        check(remainingUsers.none { it.id == id }) {
            "Deleted user with id ${id} is still present in the user pool."
        }
    }

    fun updateAndValidateUser(id: String, potentialUserUpdate: PotentialUserUpdate): User {
        val request = Request(Method.PUT, "/api/users/$id")
        val body = jsonFormatter.encodeToString(potentialUserUpdate)
        val response: Response = client(request.body(body))

        check(response.status.successful) {
            "Something went wrong updating the user with id $id:  ${response.status}: ${response.bodyString()}"
        }

        val updatedUser  = aUserLens(response)

        val updatedUser2 = getUser(id)
        check(updatedUser == updatedUser2 && updatedUser.id == "Updated $now ${id}") {
    ...
    fun createAndValidateUsers(): List<User> {
        ...
        val usersToCreate = Arb.firstName().take(config.amountOfUsers).map { it.name }.map {
            CreateUser(it, Role.entries.random(random))
        }.toList()

        val createdUsers: List<User> = usersToCreate.mapNotNull {
            ...
            if (result.isSuccess) {
                ...
                if (config.strictMode && user != fetchedUser) {
        ...
        if (config.strictMode && !allUsers.containsAll(createdUsers)) {
